...
BOOST_AUTO_TEST_SUITE(IteratorTests)
  ...
  {
    alekseev::CuckooHash< int, std::string, SimpleHash, SimpleHash, Equal > table(
        SimpleHash(), SimpleHash(), Equal(), 16);

    BOOST_CHECK(table.begin() == table.end());
  }

  BOOST_AUTO_TEST_CASE(IterateOverAllKeys)
  {
    alekseev::CuckooHash< int, std::string, SimpleHash, SimpleHash, Equal > table(
        SimpleHash(), SimpleHash(), Equal(), 16);
  ...
  BOOST_AUTO_TEST_CASE(IteratorPreIncrement)
  {
    alekseev::CuckooHash< int, std::string, SimpleHash, SimpleHash, Equal > table(
        SimpleHash(), SimpleHash(), Equal(), 16);
    ...
    auto it = table.begin();
    const auto first_key = *it;
    ++it;
    const auto second_key = *it;
    ++it;
    const auto third_key = *it;
    ++it;
    BOOST_CHECK(it == table.end());

    BOOST_CHECK(first_key != second_key && second_key != third_key && first_key != third_key);
  }

  BOOST_AUTO_TEST_CASE(IteratorPostIncrement)
  {
    alekseev::CuckooHash< int, std::string, SimpleHash, SimpleHash, Equal > table(
        SimpleHash(), SimpleHash(), Equal(), 16);
  ...
  BOOST_AUTO_TEST_CASE(IteratorDereferenceOperatorArrow)
  {
    alekseev::CuckooHash< std::string, int, SimpleHash, SimpleHash, Equal > table(
        SimpleHash(), SimpleHash(), Equal(), 16);
    ...
    for (auto it = table.begin(); it != table.end(); ++it) {
      const std::string & key_ref = *it;
      BOOST_CHECK(!key_ref.empty());

      BOOST_CHECK(it->size() > 0);
    }
  }

  BOOST_AUTO_TEST_CASE(IterationAfterRehash)
  {
    alekseev::CuckooHash< int, int, SimpleHash, SimpleHash, Equal > table(
  ...
  BOOST_AUTO_TEST_CASE(IteratorOnConstTable)
  {
    alekseev::CuckooHash< int, std::string, SimpleHash, SimpleHash, Equal > table(
        SimpleHash(), SimpleHash(), Equal(), 16);
